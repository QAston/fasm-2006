...
;        xor     ecx,ecx
	...
	push	to_main_menu			      ;powrot z procedury
default_fonts:
...
get_file_size:
;        xor     ecx,ecx
	mov	eax,4202h			      ;przenies na koniec pliku
	xor	edx,edx 			      ;pobierz rozmiar pliku
	int	21h
	shl	edx,16
	mov	dx,ax				      ;rozmiar pliku do edx
	push	edx
	mov	eax,4200h			      ;przejdz na poczatek pliku
	xor	edx,edx
	xor	ecx,ecx
	int	21h
...
to_main_menu:
	mov	[MenuPosition],0		     ;czysc pozycje menu
to_main_loop:
...
SizeBuffer = 1+(80*50*2)+32h+36+(257*2)+(65*2)-4
