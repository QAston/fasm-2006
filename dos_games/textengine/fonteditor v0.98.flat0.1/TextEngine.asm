format mz
;stack  100h
;engine textowy w DPMI  wersja flat
include 'loader.inc'
...
code_offs:
	...
	shl	esi,4
	sub	edx,esi
	mov	[ASegmentBase],esi


	movzx	eax,word[cs:edx]		    ;pobierz offset fontow 8/8
	movzx	ebx,word[cs:edx+2]		    ;pobierz segment
	shl	ebx,4
	add	eax,ebx 			      ;oblicz adres rzeczywisty
	sub	eax,esi
	mov	[ActualFonts],eax		     ;zapisz adres
...
.font_low:
	mov	eax,[ActualFonts]		 ;pobierz adres ze zdefiniowanymi fontami
	shl	ebp,3
	add	eax,ebp 			  ;eax= adres potrzebnego fonta
	mov	edi,Font
@@:
	mov	ebx,[eax]		       ;pobierz czesc fonta
...
TProgramTitle:	db 'GreenT Text Game Reader Font Editor v 0.9.8.',0
...
TAuthor:
		...
		db 'Autorem programu jest QAston <jeszcze nie ma adresu>.',10,13
		db 'Autor jest cz',≥,'onkiem Green Grass Studio(c)2007.',10,13
		db 'GreenT Text Game Reader Font Editor jest cz',Í,ú,'ci',π,' pakietu GreenT Tools.',10,13
		db 'GreenT Text Game Reader(c) oraz GreenT Tools(c) s',π,' rozpowszechniane na' ,10,13,'licencji freeware. '
...
FontEditor:		   db	   0dbh,0dfh,0dbh,0ddh,020h,0deh,0dbh,0dch,0dbh 	  ;ramka edytora fontow :P
FONTS:
NewFontUp		   db	   30,0,0,0,18h,3ch,7eh,0ffh,0ffh			  ;domyslne fonty
