...
include 'fontuse.inc'
	...
	mov	edx,NazwaPliku
	call	load_from_file_to_memory
	mov	esi,edx

	mov	edi,AnimAlocHandle
	xor	bx,bx
...
draw_comp_anim_frame:
	mov	edi,BVideo
	mov	esi,[ds:ebp+animaddr]
	movzx	eax,byte[esi+settings]		  ;al-ustawienia
	ror	eax,16
;        test    eax,10000h
;        jne     .color_not_predefined
;        mov     ah,[esi+1]
.color_not_predefined:
	mov	ecx,[ds:ebp+redrawframe]
	mov	edx,[ds:ebp+actualframe]
	test	edx,edx
	jne	@f
	add	esi,13
	mov	[ds:ebp+redrawaddr],esi
	jmp	.first_frame
	@@:
	mov	esi,[ds:ebp+redrawaddr]
.first_frame:
	sub	edx,ecx
.draw_animation_frame:
	inc	esi;bajt redrawu ramki
	mov	bx,[ds:ebp+position]
;        test    eax,10000h
;        je      .draw_line
	mov	ah,[esi]
	inc	esi
...
.animation_done:
	...
	jz	.animations_done
	dec	edx
	jmp	.draw_animation_frame
.animations_done:
	mov	[ds:ebp+lastaddr],esi
	ret
...
load_from_file_to_memory:
	;edx-adres nazwy pliku
.open_file:
...
;       jc      ending
	...
	shl	ebx,16				      ;oblicz adres pliku wzgledem code_seg
	mov	eax,[CodeSegmentBase]
	mov	bx,cx
	sub	ebx,eax
	mov	[ebx],si
