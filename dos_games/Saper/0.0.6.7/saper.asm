...
draw_comp_anim_frame:
	...
	mov	edx,[ds:ebp+4]
	add	esi,5
	movzx	eax,byte[esi]		 ;al-ustawienia
	ror	eax,16
	mov	ecx,-1
	test	eax,10000h
	jne	.goto_frame
...
.goto_frame_loop:
	mov	bx,[esi]
	test	bl,bl		; \ jesli tak to dodaj 3
	jnz	.no_special
	test	bh,bh	     ;jesli tak to dodaj do esi 2
	je	.next_
	cmp	bh,07dh       ;jesli tak to dodaj 3
	je	.next
	test	bh,10000000b   ; /
	jz	.no_special
.next:
