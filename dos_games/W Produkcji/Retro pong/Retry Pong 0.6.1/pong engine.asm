...
;menu
;*******************************************************************
menu:		mov	ax,0003h       ;wybor opcji gry
		int	10h
		mov	ah,09h
		mov	dx,TMenu
		int	21h
...
zmienINT9:	push	bx dx es ax		  ;zmien adres int 9
		mov	al,09h
		mov	ah,35h
		int	21h
		mov	word[staryint9],bx	   ;zachowaj stary adres
		mov	word[staryint9+2],es
		mov	al,09h
		mov	dx,scankeyboard
		mov	ah,25h
		int	21h
...
czekaj: 	mov	fs,[gs:6ch]

czekpetla:	mov	cx,[gs:6ch]
		mov	ax,fs
		sub	cx,ax
		cmp	cx,1
		jb	czekpetla
...
domenu: 				      ;wyzeroj zmienne przed wyjsciem do menu
		...
		mov	byte[poziomAI],0
		push	es ds
		mov	al,09h			  ;przywroc int 09
		mov	dx,word[staryint9]
		mov	ds,word[staryint9+2]
		mov	ah,25h
		int	21h
		mov	al,08h			  ;przywroc int 08
		mov	dx,word[staryint8]
		mov	ds,word[staryint8+2]
		mov	ah,25h
		int	21h
		pop	ds es
		ret
