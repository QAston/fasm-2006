...
LiczbaC db 200
bufor: times 45 db 0
pytanko db 'Czy chcesz zachowaæ swoje ¿yczenia i zakoñczyæ program? Jeœli wciœniesz [nie] to usuniesz swoje i obejrzysz moje ¿yczenia.',0
PZyczenia db 'data\zyczenia.txt',0
...
pytanie db 'Czy chcesz jeszcze raz? Jak ¿yczenia Ci siê nie spodoba³y to je ANULUJ i wpisz swoje! (Tylko 200znaków!)',0
TButton db 'OK!'
...
endp
proc window1
invoke CreateFile,PZyczenia,GENERIC_READ,0,0,OPEN_EXISTING,0,0
mov    [HWRITE],eax
invoke GMH,0
mov [hInstance],eax
...
endl
	...
	jmp .pkom
    .czytzycz:
	invoke ReadFile,[HWRITE],wyswietl,201,LiczbaC,0
	invoke CloseHandle,[HWRITE]
	cmp    byte[wyswietl],0
	je     .wmdalej
	invoke MessageBox,[hwnd],wyswietl,tytul,MB_OK or MB_DEFBUTTON1 or MB_ICONWARNING
	invoke MessageBox,[hwnd],pytanko,tytul,MB_YESNO+MB_ICONQUESTION
	cmp eax,IDNO
	jne .wmdestroy
	invoke DeleteFile,PZyczenia
	.wmdalej:
	mov [HWRITE],0
	dec [licz]
	jmp .wmcommand
...
endl
	...
	cmp	ax,Button
	jne	.finish
	invoke GetWindowText,[hEdit],nzyczenia,200
	test   eax,eax
	jz     .finish
	mov    ebx,eax
	lea    esi,[nzyczenia]
	...
	jnz	.pnzyczenia
	invoke MessageBox,[hwnd],wyswietl,tytul,MB_OK or MB_DEFBUTTON1 or MB_ICONWARNING
	invoke CreateFile,PZyczenia,GENERIC_WRITE,FILE_SHARE_WRITE,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0
	invoke WriteFile,eax,wyswietl,200,Liczba,0
	invoke ExitProcess,0
    .wmdestroy:
	invoke PQM,0
    .finish:
