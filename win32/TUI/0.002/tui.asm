...
make_delay:				;do eax dlugosc przedzialu czasowego
	mov  ebx, 6629
	mov  ecx, 100
	mul  ebx
	div  ecx
...
file_to_mem:
;podaj wskaznik do nazwy pliku w esi
	mov    edx,esi
	xor    al,al
	call   open_file
	mov    ebx,eax
...
open_file:
;edx, wskaznik na nazwe
;al tryb otwarcia bit 001 write only 000 read only 010 read/write
;zwraca eax uchwyt do pliku
	push	cx
	mov	ah, 3dh
	xor	cx,cx
	int	21h
